...
- Add a todo: title (required), description, priority (low / medium / high), due date, comma-separated tags
- List each todo: checkbox, title, priority colour, due date, tags
...
- Delete from the row, with an in-app confirm
...
- Overdue dates are highlighted if the todo is still open

## Ticket page (`/todo.html?id=`)

- Reads `id` from the query string, not from a path segment
- Fetches `GET /api/todos/:id`
- Shows title, description, priority, due date, tags, created at, updated at, open/done
- Edit in place and Save; the button shows Saved for a moment after a successful write
- Mark done / reopen
- Delete, with an in-app confirm, then the browser goes back to the list
...
- `GET /api/todos/:id` — one todo (this is what the details page uses)
...
- class-validator on write DTOs: title length, enum priority, ISO dates, max 8 tags
